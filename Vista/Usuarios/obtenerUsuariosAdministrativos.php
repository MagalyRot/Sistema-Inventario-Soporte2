...
$output= array();
$sql = "SELECT * FROM vista_administrativos ";
...
while($row = mysqli_fetch_assoc($query))
{
	...
	if($row['estado']==1){
		...
		$sub_array[] = '<a href="javascript:void();" data-id="'.$row['id'].'"  class="btn app-btn-secondary btn-sm editbtnadmin" >Editar</a>' ; 
		$sub_array[] = '<a href="javascript:void();" data-id="'.$row['id'].'"  class="btn app-btn-secondary btn-sm desactivarDatosBtn" >Desactivar</a>';
		
	}else{
		$sub_array[] = '<span class="badge bg-danger">Inactivo</span>';
		$sub_array[] = '<a href="javascript:void();" data-id="'.$row['id'].'"  class="btn app-btn-secondary btn-sm editbtnadmin" >Editar</a>' ; 
		$sub_array[] = '<a href="javascript:void();" data-id="'.$row['id'].'"  class="btn app-btn-secondary btn-sm ActivarDatosBtn" >Activar</a>';
	}$data[] = $sub_array;
}
