...
function obtener_todos_registros(){
    include("Conexion.php");
    $stmt = $conexion->prepare("SELECT *FROM vista_asignacionescanceladas"); 
    $stmt->execute();
    ...
    $query = '';
    $salida = array();   
    $query = "SELECT * from vista_asignacionescanceladas ";
    $columns = array('folio','fechaI','ubi','persona','tipo');
    ...
    $sub_array[] = $fila["ubi"];
    $sub_array[] =$fila["tipo"];
    $sub_array[] = '<span class="badge bg-danger" id="' . $fila["estado"] . '">Cancelado</span>';
    $datos [] = $sub_array;     
