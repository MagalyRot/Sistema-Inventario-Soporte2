...
if (!$equipos) {
    header("Location: ./prestamos1.php?status3=4");
    exit;
}
...
$indice = false;
for ($i = 0; $i < count($_SESSION["carritoEquipos2"]); $i++) {
    if ($_SESSION["carritoEquipos2"][$i]->codigo === $codigo) {    
        $indice = $i;
        break;
    }
}
...
if ($indice === false) {
   ...
   // $producto->total = $producto->precioVenta;
    array_push($_SESSION["carritoEquipos2"], $equipos);
} else {
    # Si ya existe, se agrega la cantidad
    # Pero espera, tal vez ya no haya
    $cantidadExistente = $_SESSION["carritoEquipos2"][$indice]->cantidad;
    # si al sumarle uno supera lo que existe, no se agrega
    if ($cantidadExistente + 1 > $equipos->existencia) {
        header("Location: ./prestamos1.php?status3=5");
        exit;
    }
    $_SESSION["carritoEquipos2"][$indice]->cantidad++;
